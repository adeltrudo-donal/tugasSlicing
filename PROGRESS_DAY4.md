...
const FavoriteScreen = ({ navigation }) => {
  ...
  return <FlatList data={favorites} renderItem={renderCafeCard} />;
};
```
...
### Distance Calculation from `latlong`

Setiap cafe dari API punya field `latlong` berbentuk `latitude;longitude`. Di layer thunk, nilai ini diubah menjadi jarak terhadap koordinat origin yang di-hard-code di file action. Hasilnya disimpan ke field `distance` dengan format seperti `1.4 km away`.

Alur teknisnya:

1. API mengembalikan data cafe mentah.
2. `cafe_action.ts` membaca `latlong` setiap item.
3. Koordinat cafe dibandingkan dengan `CURRENT_LOCATION` menggunakan rumus Haversine.
4. Hasil jarak diformat menjadi string dan ditempel ke object cafe.
5. UI cukup menampilkan `cafe.distance` tanpa menghitung ulang.
